...
export const FindUsernamePage: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const response = await api.post('/members/username-recovery', { email });
      setMessage(response.data?.message || '가입 이메일로 아이디 안내를 전송했습니다.');
    } catch (err: any) {
      setError(err.response?.data?.message || '아이디 찾기 요청에 실패했습니다.');
    }
  };
  ...
  return (
    <Card className="mx-auto mt-10 max-w-md p-6">
      ...
      <form onSubmit={submit} className="mt-6 space-y-5">
        ...
        <Button type="submit" className="w-full">아이디 안내 받기</Button>
      </form>
